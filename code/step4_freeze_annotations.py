#!/usr/bin/env python
"""Freeze final-cluster annotations without duplicating the large source H5AD.

The v21 H5AD remains the immutable expression/count source.  This script writes a
cell-level metadata sidecar and cluster-level audit tables.  A future COMPASS
input must be generated by joining the sidecar to the source H5AD by cell_id.
"""
...
def main() -> None:
    ...
    cluster_summary.to_csv(args.tabledir / "cluster_annotation_evidence.tsv", sep="\t", index=False)
    definite = cluster_summary.loc[
        cluster_summary["change_class"].eq("definite_correction"),
        ["cluster_id", "n_singlets", "old_labels", "cell_type_v22", "cell_state_v22",
         "annotation_confidence", "positive_marker_evidence", "interpretation_note"],
    ].copy()
    definite.to_csv(args.tabledir / "annotation_corrections.tsv", sep="\t", index=False)
    if old_col in obs.columns:
        transition = (
            obs.loc[obs["is_singlet_v22"]]
            .groupby([old_col, "cell_type_v22"], observed=True)
            .size()
            .reset_index(name="n_singlets")
            .sort_values([old_col, "n_singlets"], ascending=[True, False])
        )
        transition.to_csv(args.tabledir / "annotation_transition.tsv", sep="\t", index=False)
    ...
    payload = {
        ...
        "expression_contract": {
            ...
            "future_compass": "deferred: no COMPASS group membership is defined by this Step 1-4 freeze",
        },
        "large_file_policy": "No duplicate H5AD written; join metadata/cell_metadata.tsv.gz to the immutable source by cell_id.",
    }
